...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    un_viscorner = [item for item in corners if item not in (0,0)if state[1]]
    hn = todo(un_viscorner, state[0])
    return hn


def todo(corners, pos):
    if len(corners) == 0:
        return 0
    hn = []
    for loc in corners:
        dis = abs(loc[0] - pos[0]) + abs(loc[1] - pos[1]) + todo(
            [c for c in corners if c != loc], loc)
        hn.append(dis)
    return min(hn)
...
class ApproximateSearchAgent(Agent):
    ...
    def registerInitialState(self, state):
        ...
        self.corners = ((1, 1), (1, top), (right, 1), (right, top))
        corners_path = [((mazeDistance(state.getPacmanPosition(), c, state), c)) for c in
                        self.corners]
        prob = PositionSearchProblem(state, start=state.getPacmanPosition(),
                                     goal=min(corners_path)[1], warn=False)
        self.moves = search.bfs(prob)
        self.foodGrid = state.getFood()
        start = state.getPacmanPosition()
        mcdonalds = []
        for x, row in enumerate(self.foodGrid):
    ...
    def adjacent_dots(self, state, currx, curry):
        foodGrid, walls = state.getFood(), state.getWalls()
        count = 0
        ...
        if not self.moves:
            ...
            if not mcdonalds:
                for x, row in enumerate(foodGrid):
                    for y, cell in enumerate(row):
                        if foodGrid[x][y]:
                            score = mazeDistance(state.getPacmanPosition(), (x, y),
                                                 state), self.adjacent_dots(state, x, y), (x, y)
                            mcdonalds.append(score)
            if mcdonalds:
                coordinate = min(mcdonalds)[2]
                prob = PositionSearchProblem(state, start=state.getPacmanPosition(),
                                             goal=coordinate, warn=False)
