...
def cornersHeuristic(state, problem):
    ...
    cornerLeft = []
    for cor in corners:
        if cor not in cornerList:
            cornerLeft.append(cor)
    ...
    while cornerLeft:
        min_dist = -1
        min_dist_cor = None
        for cor in cornerLeft:
            dist = util.manhattanDistance(currentPos, cor)
        ...
        cost += min_dist
        #print cost
        currentPos = min_dist_cor
        cornerLeft.remove(min_dist_cor)

        if extreme_dist < min_dist:
            extreme_dist = min_dist 

    return max(cost - extreme_dist, 0)
...
def foodHeuristic(state, problem):
    ...
    while foodPos:
        ...
        for pos in foodPos:
            dist = util.manhattanDistance(currentPos, pos)
            if min_dist == -1 or min_dist > dist:
                min_dist = dist
                min_dist_pos = pos

        cost += min_dist
        currentPos = min_dist_pos
        foodPos.remove(min_dist_pos)
        if extreme_dist < min_dist:
            extreme_dist = min_dist


    return max(cost - 0.5 * extreme_dist, 0)# Default to trivial solution
    #return 0
